...
void onMessage(const FIX44::MarketDataRequest &request, const FIX::SessionID &sessionID) {
    ...
    // Append new symbols instead of overwriting
    {
        std::lock_guard<std::mutex> lock(subMutex);
    ...
    {
        std::lock_guard<std::mutex> lock(subMutex);
        for (auto &s : subscribedSymbols[sessionID])
            std::cout << " " << s;
    }
    std::cout << std::endl;
}
...
private:
    ...
    std::unordered_map<std::string, double> contractSizes{
        {"GOLD", 100}, {"GOLD_DEC25", 100}
    };

    private:
    std::unordered_map<std::string, std::pair<double,double>> lastSentPrices; // symbol -> {bid, ask}
    ...
    void broadcastPriceToFIX(const std::string &priceData) {
        ...
        while (std::getline(ss, token, '\n')) {
            ...
            if (symbolPart.empty() || !isalpha(symbolPart[0])) continue;

            bidPart.erase(std::remove(bidPart.begin(), bidPart.end(), ';'), bidPart.end());
            askPart.erase(std::remove(askPart.begin(), askPart.end(), ';'), askPart.end());
            ...
            // Skip if prices are unchanged
            auto itLast = lastSentPrices.find(symbol);
            if (itLast != lastSentPrices.end() && itLast->second.first == bid && itLast->second.second == ask)
                continue;
            ...
            for (const auto &sessID : sessions) {
                ...
                try { FIX::Session::sendToTarget(mdSnap, sessID); }
